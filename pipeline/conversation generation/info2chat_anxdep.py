...
def info2chat(case_description):
    ...
    2) Include all symptoms of the client in the conversation
    3) Do not generate any information that is not in the case description
    4) Range of rounds of interaction: LESS THAN 8 ROUNDS!!!!
    5) If the subject is a child or young adolescent, the only client spearker should be the subject's parent in the conversation (only one client can talk).
...
for i in tqdm(range(0,len(case_test_file_anxdep['extracted_info']),1)):
    case_description = case_test_file_anxdep['extracted_info'].iloc[i]
    conv = info2chat(case_description)
    case_ls.append(conv)

# save the data
raw_save = pd.read_excel("anxdep_cases_v2_save.xlsx")
raw_save["raw_conversation"] = case_ls
raw_save.to_excel("anxdep_cases_v2_save.xlsx", index=False)

...
os.chdir('your path')
case_test_file_other = pd.read_excel("other_disorders_cases_v2.xlsx")
...
for i in tqdm(range(0,len(case_test_file_anxdep),1)):
    output = response_return(system_role_, " Case description: " + case_test_file_other["case description"][i] + format_prompt_)
    case_descript_ls.append(output)

# save the extracted information

save = pd.read_excel("other_disorders_cases_v2_save.xlsx")
save["extracted_info"] = case_descript_ls
save.to_excel("other_disorders_cases_v2_save.xlsx", index = False)
